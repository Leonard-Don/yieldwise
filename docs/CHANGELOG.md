...
记录主要迭代节奏。每条目对应一个或多个 PR/合并 commit。

历史 agent 规划/spec 底稿已从仓库移除；这里仅保留已经落地的产品与工程变更。

---

## 2026-05-05 — v1.0 自用闭环版：功能开发冻结

- 将项目状态收口为 **v1.0 self-use final / 自用闭环版**，版本号升至 `1.0.0`
- 明确后续不再做新功能开发；仅在阻塞性 bug、数据质量问题或真实使用中反复出现的同一痛点出现时重开
- 当前自用闭环包括：地图发现机会、详情证据、候选研究台、目标触发 alerts、候选复核动作、结构化 Markdown 决策备忘录、后台刷新中心、公开采样 / review queue / 几何 QA
- 闭环验证基线沿用完整测试与浏览器回归：后端 / 前端测试、ruff、compileall、`full_browser_regression.py` 27 条浏览器断言

## 2026-05-05 — Candidate desk: 候选研究与复核闭环

- 新增候选研究台：支持小区 / 楼栋 / 区级候选，按到期复核、目标触发、价格 / 样本变化、证据缺口和 shortlist 分组
- 候选字段扩展为状态、优先级、投资假设、目标价、目标租金、目标收益率、复核日期、最后复核时间和本地 notes
- 新增候选动作接口：复核完成、延后复核、加入 shortlist、标记放弃；复核完成会写回当前快照基线
- alerts 接入候选目标触发、到期复核、证据不足、同楼层新样本变化等信号
- 决策备忘录升级为投决记录结构：投资假设、买入理由、反对理由、证据来源、待核验项和下一步动作
