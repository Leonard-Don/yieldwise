...
def _browser_capture_run_detail_cached(run_id: str) -> dict[str, Any] | None:
    ...
    return {
        **run_summary,
        ...
        "attentionCount": len(attention_rows),
    }
...
def _browser_capture_task_history_index_cached() -> dict[str, dict[str, Any]]:
    ...
    for item in capture_runs:
        ...
        if current and (current.get("createdAt") or "") >= (item.get("createdAt") or ""):
            continue
        latest_by_task[task_id] = item
    for task_id, item in latest_by_task.items():
        history_count = grouped_counts.get(task_id, 0)
        attention_count = int(item.get("attentionCount") or 0)
...
def available_geo_baseline_runs_for(run_id: str) -> list[dict[str, Any]]:
    ...
    selected_created_at = selected_run.get("createdAt") or ""
    return [
        item
        for item in runs
        ...
        and (item.get("createdAt") or "") < selected_created_at
    ]
...
def baseline_run_for(run_id: str, baseline_run_id: str | None = None) -> dict[str, Any] | None:
    ...
    if baseline_run_id:
        ...
        if not explicit_baseline:
            return None
        if (explicit_baseline.get("createdAt") or "") >= (current_run.get("createdAt") or ""):
            return None
    ...
    same_provider_candidates = [
        ...
        and (item.get("createdAt") or "") < (current_run.get("createdAt") or "")
    ]
    ...
    for item in runs:
        if item["runId"] == run_id:
            continue
        if (item.get("createdAt") or "") < (current_run.get("createdAt") or ""):
            return item
    return None
    ...
    *,
    ...
    return {
        ...
        "summary": {
            ...
            "attentionTaskCount": sum(1 for item in items if int(item.get("latestCaptureAttentionCount") or 0) > 0),
...
def submit_browser_sampling_capture(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "task": task_lookup or task_snapshot or {},
    }
